...
import type { Floor, Dimension, Type, Types, UUID } from '../types.js'
...
export class FloorPlanManager {
	...
	add_dimensions = ({ type_id, floor_id }: { type_id: UUID; floor_id: UUID }) => {
		const floor = this.content.types
			.find((type) => type.id === type_id)
			?.floors.find((floor) => floor.id === floor_id)
		if (!floor) return
		...
		floor.dimensions.push({
			...this.default_type.floors[0].dimensions[0],
			name: `Type ${floor.dimensions.length + 1}`,
			id: crypto.randomUUID(),
		})
	...
	delete_dimension = ({
		...
		if (!confirm('Are you sure you want to delete these dimensions?')) return
		...
		floor.dimensions = floor.dimensions.filter((dimension) => dimension.id !== dimension_id)
	...
	add_room = ({
		...
		const dimension = floor.dimensions.find((dimension) => dimension.id === dimension_id)
		if (!dimension) return

		dimension.rooms.push({
			...this.default_type.floors[0].dimensions[0].rooms[0],
			name: `Room ${dimension.rooms.length + 1}`,
			id: crypto.randomUUID(),
		})
	...
	delete_room = ({
		...
		dimension.rooms = dimension.rooms.filter((room) => room.id !== room_id)
