...
{#if themes?.length}
	<fieldset>
		<label class:selected={!content}>
			<input type="radio" value={undefined} bind:group={content} onchange={update} />
			None
		</label>
		{#each themes as { value, name, hex }}
			<label class:selected={content === value} style="--color: {hex}">
				<svg class="size-4" viewBox="0 0 183 184" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path
				...
				<input type="radio" {value} bind:group={content} onchange={update} />
				{name}
			</label>
		{/each}
	</fieldset>
